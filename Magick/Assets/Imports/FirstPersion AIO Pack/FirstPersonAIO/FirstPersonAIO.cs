...
public class FirstPersonAIO : MonoBehaviour {
    ...
    private void Start(){
        ...
        if(autoCrosshair || drawStaminaMeter){
            ...
            if(autoCrosshair){
                ...
                crossHair.rectTransform.sizeDelta = new Vector2(40,40);
    ...
    private void Update(){
        ...
        #region Look Settings - Update
            ...
            if (cameraInputMethod == CameraInputMethod.Traditional || cameraInputMethod == CameraInputMethod.TraditionalWithConstraints){
                    mouseYInput = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.X ? controls.Player.LookY.ReadValue<float>() : -controls.Player.LookY.ReadValue<float>();
                    mouseXInput = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? controls.Player.LookX.ReadValue<float>() : -controls.Player.LookX.ReadValue<float>();
            }
            else{
                mouseXInput= 0 * (mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? 1 : -1);
        ...
        if(canHoldJump ? (canJump && controls.Player.Jump.triggered) : (controls.Player.Jump.triggered && canJump) ){
            jumpInput = true;
        }else if(controls.Player.Jump.triggered) {jumpInput = false;}
        
        
        if(_crouchModifiers.useCrouch){
            if(!_crouchModifiers.toggleCrouch){ isCrouching = _crouchModifiers.crouchOverride || controls.Player.Crouch.triggered; }
            else if(controls.Player.Crouch.triggered) {isCrouching = !isCrouching || _crouchModifiers.crouchOverride;}
            }

        if(controls.UI.PauseButton.triggered) {ControllerPause();}
        #endregion
    ...
    private void FixedUpdate(){
        ...
        if(useStamina){
            isSprinting = controls.Player.Sprint.triggered && !isCrouching && staminaInternal > 0 && (Mathf.Abs(fps_Rigidbody.velocity.x) > 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z) > 0.01f);
            if(isSprinting){
            ...
            }else if((!controls.Player.Sprint.triggered || Mathf.Abs(fps_Rigidbody.velocity.x)< 0.01f || Mathf.Abs(fps_Rigidbody.velocity.z)< 0.01f || isCrouching)&&staminaInternal<staminaLevel){
